...
export default function AdminEmergencyNeed() {
  ...
  async function load() {
    ...
    setError(null);
    try {
      const res = await fetch('/api/emergency-request?all=1', {
        cache: 'no-store',
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 text-gray-900">
      <div className="mx-auto max-w-6xl px-4 py-10 space-y-10">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex gap-3">
            <button
              ...
              className="rounded-lg bg-gray-800 text-white px-4 py-2 text-sm font-medium hover:bg-gray-700 transition"
            >
              Dashboard
            </button>
            <button
              onClick={load}
              disabled={loading}
              className="rounded-lg bg-indigo-600 text-white px-4 py-2 text-sm font-medium hover:bg-indigo-700 transition disabled:opacity-60"
            >
                ...
                >
                  <div className="flex flex-col md:flex-row gap-4">
                    {r.photo_url && (
                      ...
                      <img
                        src={r.photo_url}
                        alt={`Photo for ${r.name}'s request`}
                    ...
                    <div className="flex flex-col gap-2 justify-between">
                      ...
                      >
                        {updating === r.id ? 'Approving…' : 'Approve'}
                      </button>
                    </div>
                  </div>
                </li>
        ...
        <section className="rounded-2xl border border-gray-200 bg-white shadow-sm p-5">
          <h2 className="text-sm font-semibold text-gray-700 mb-4">
            Approved (Visible Public) ({approved.length})
          </h2>
          {approved.length === 0 ? (
            <div className="text-xs text-gray-500">
