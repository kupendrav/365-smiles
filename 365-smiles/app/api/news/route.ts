    ...
    const base = [
      {
        ...
        summary:
          'Local volunteers organized a support drive for senior citizens providing meals and medical check-ups.',
        source: '365 Smiles',
      ...
      },
      {
        ...
        summary:
          'New school year starter kits (books & stationery) were delivered to children at a partner orphanage.',
        source: '365 Smiles',
      ...
      },
      {
        ...
        summary:
          'Emergency funds allocated for a child requiring urgent treatment earlier today.',
        source: '365 Smiles',
      ...
      },
      {
        ...
        summary:
          'Meal sponsorship program added two new distribution points increasing daily reach.',
        source: '365 Smiles',
      ...
      },
      {
        ...
        summary:
          'New volunteers received training on donation logging and beneficiary interaction protocols.',
        source: '365 Smiles',
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (kind === 'combined') {
      ...
      try {
        cseData = await fetchCombinedNews({ limit, recentDays: 30 });
      } catch (e) {
      ...
      if (cseData && cseData.length > 0) {
        return apiSuccess({
          kind,
          count: cseData.length,
          data: cseData,
          source: 'worldnews',
          debug: { worldnews: 'ok', count: cseData.length },
        });
      }
      ...
      if (!mapped.length) {
        ...
        return apiSuccess({
          ...
          note: 'Dummy news used (combined fallback).',
        });
      }
    ...
    if (kind === 'emergency') {
      ...
      try {
        worldData = await fetchEmergencyNews({ limit, recentDays: 7 });
      } catch (e) {
      ...
      if (worldData && worldData.length > 0) {
        return apiSuccess({
          ...
          data: worldData,
          source: 'worldnews',
          debug: { worldnews: 'ok', count: worldData.length },
        });
      }
      ...
      if (!mapped.length) {
        ...
        return apiSuccess({
          ...
          note: 'Dummy news used (emergency fallback).',
        });
      }
