...
export default function RecentDonors() {
  ...
  const loadMore = async () => {
    ...
    setLoadingMore(true);
    const lastId = donors[donors.length - 1]?.id;
    const { data, error } = await supabase
      .from('public-donations')
      .select('*')
      .order('id', { ascending: false })
    ...
    if (!error && data) {
      setDonors((prev) => [...prev, ...(data as Donor[])]);
      if (data.length < PAGE_SIZE) setHasMore(false);
    }
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="h-40 bg-gray-800 rounded-2xl animate-pulse"
            />
          ))}
        </div>
      ) : (
        <>
          ...
          {/* Pagination: Load More (Issue #18) */}
          {hasMore && (
            <div className="mt-12 text-center">
              <button
                onClick={loadMore}
                disabled={loadingMore}
                className="inline-block rounded-full border border-pink-400 px-8 py-3 text-pink-400 hover:bg-pink-400 hover:text-white transition font-semibold disabled:opacity-60"
              >
